 ...
 *
 * Genera reporte de drivers inactivos en un rango de fechas para una ciudad específica.
 ...
 */
@Component({
  selector: 'app-report-deliveries-inactivos',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './report-deliveries-inactivos.component.html',
  providers: [MessageService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReportDeliveriesInactivosComponent implements OnInit {
  ...
  generateReport(): void {
    ...
    this.service.getReportDeliveryInactive(startDate, endDate, cityId!, format!).subscribe({
      next: (base64) => {
        const selectedCity = this.cityList().find(c => c.cityId === cityId);
        const cityName = selectedCity?.cityName || 'ciudad';
        const filename = `reporte-deliveries-inactivos_${startDate}_${endDate}_${cityName}`;
