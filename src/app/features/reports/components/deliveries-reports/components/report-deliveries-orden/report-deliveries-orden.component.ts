 ...
 *
 * Reporte #7: Deliveries por Orden (Cantidad de Envíos)
 *
 * Genera reporte de cantidad de envíos por driver en un rango de fechas para una ciudad específica.
 ...
 */
@Component({
  selector: 'app-report-deliveries-orden',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './report-deliveries-orden.component.html',
  providers: [MessageService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReportDeliveriesOrdenComponent implements OnInit {
  ...
  generateReport(): void {
    ...
    this.service.getReportDeliveryDriver(startDate, endDate, cityId!, format!).subscribe({
      next: (base64) => {
        const selectedCity = this.cityList().find(c => c.cityId === cityId);
        const cityName = selectedCity?.cityName || 'ciudad';
        const filename = `reporte-deliveries-orden_${startDate}_${endDate}_${cityName}`;
