 ...
 *
 * Reporte #2: Deliveries Activos
 *
 * Genera reporte de drivers activos para una fecha específica y ciudad.
 ...
 */
@Component({
  selector: 'app-report-deliveries-activos',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './report-deliveries-activos.component.html',
  providers: [MessageService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReportDeliveriesActivosComponent implements OnInit {
  ...
  generateReport(): void {
    ...
    this.service.getReportActiveDrivers(formattedDate, cityId!, format!).subscribe({
      next: (base64) => {
        const selectedCity = this.cityList().find(c => c.cityId === cityId);
        const cityName = selectedCity?.cityName || 'ciudad';
        const filename = `reporte-deliveries-activos_${formattedDate}_${cityName}`;
