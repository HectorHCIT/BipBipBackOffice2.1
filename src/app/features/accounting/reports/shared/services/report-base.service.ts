...
import { Brand, Store, City } from '../../models/report-common.types';
 ...
 * - Métodos para obtener datos de referencia (brands, stores, cities)
 * - Métodos de formateo de fechas
 *
 * Patrón: Cada reporte extiende este servicio e implementa su método generateReport()
 *
 * @example
 * @Injectable({ providedIn: 'root' })
 * export class CashFlowService extends ReportBaseService {
 *   generateReport(date: Date, store: string, storeId: number): Observable<string> {
...
export abstract class ReportBaseService {
  ...
  /**
   * Método abstracto que cada reporte debe implementar
   * Retorna Observable<string> con el base64 del archivo generado
   */
  abstract generateReport(...params: any[]): Observable<string>;

  /**
   * Obtiene lista de marcas/brands ordenadas
  ...
  /**
   * Obtiene tiendas por marca
   * Endpoint: Restaurant/shortNames?brandId={brandId}
   *
   * @param brandId - ID de la marca
   */
  getStoresByBrand(brandId: number): Observable<Store[]> {
    return this.dataService.get$<Store[]>(`Restaurant/shortNames?brandId=${brandId}`);
  }

  /**
   * Obtiene lista completa de ciudades
   * Endpoint: Location/CityList
   */
